...
class Tickets extends Database
{
	...
	public function showTickets()
	{
		...
		while ($ticket = mysqli_fetch_assoc($result)) {
			...
			$ticketRows[] = '<button type="button" name="delete" subject="'.$ticket["title"].'" id="' . $ticket["id"] . '" class="btn btn-danger btn-xs delete"  ' . $disbaled . '>Close</button>';
	...
	public function createTicket()
	{
		if (!empty($_POST['subject']) && !empty($_POST['message'])) {
			try {
				...
				$queryInsert = "INSERT INTO " . $this->ticketTable . " (uniqid, user, title, init_msg, department, date, last_reply, user_read, admin_read, resolved) VALUES('" . $uniqid . "', '" . $_SESSION["userid"] . "', '" . $_POST['subject'] . "', '" . $message . "', '" . $_POST['department'] . "', '" . $date . "', '" . $_SESSION["userid"] . "', 0, 0, '" . $_POST['status'] . "')";
				mysqli_query($this->dbConnect, $queryInsert);
				// Send mail to ticket creator
				$this->mail->sendEmail($user_info['email'], $user_info['name'], ["subject" => $_POST['subject'], "msg" => $message]);
				echo 'success ' . $uniqid;
			} catch (Exception $e) {
	...
	public function updateTicket()
	{
		if ($_POST['ticketId']) {
			try {
				...
				$updateQuery = "UPDATE " . $this->ticketTable . " SET title = '" . $_POST["subject"] . "', department = '" . $_POST["department"] . "', init_msg = '" . $_POST["message"] . "', resolved = '" . $_POST["status"] . "'WHERE id ='" . $_POST["ticketId"] . "'";
	...
	public function closeTicket()
	{
		if ($_POST["ticketId"]) {
			try {
				...
				$this->mail->sendTicketStatusEmail($user_info['email'], $user_info['name'], ["subject" => $_POST['subject'], "msg" => "Resolved"]);
			} catch (Exception $e) {
				echo 'Error: ' . $e->getMessage();
			}
		}
	...
	public function saveTicketReplies()
	{
		...
		if ($_POST['message']) {
			try {
				...
				$this->mail->sendTicketStatusEmail($user_info['email'], $user_info['name'], ["subject" => $_POST['subject'], "msg" => $message]);
			} catch (Exception $e) {
				echo $e->getMessage();
			}
		}
